...
void radixsort_parallel(uint32_t *input_array, uint32_t input_array_size,
				uint32_t max_digits_size, int debug, int chunksize, int threadNum)
{
	uint32_t digit;
	int f, i, j;
	double start_time;
...
int main(int argc, char **argv)
{
	...
	if (argc >= 2) {
		//num_of_values = strtoul(power, NULL, 10);
		num_of_values =pow(2,power);
		if (num_of_values < 0 || num_of_values > UINT32_MAX)
			num_of_values = 1000;
	}
	
	auto threadNum = strtol(argv[2],NULL,10);
	...
	start_time = omp_get_wtime();
	radixsort_parallel(input, num_of_values, max_digits_size, debug, chunksize, threadNum);
	printf("CZAS SORTOWANIA:\t%f", omp_get_wtime()-start_time);
	if (debug > 0)
		printf("\nK\t\tN\tTIME\n");
